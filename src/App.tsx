...
const MainLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-rival-dark text-white selection:bg-rival-teal selection:text-rival-dark font-sans flex flex-col relative cursor-none overflow-x-hidden">
      <SmoothCursor />
      {/* Global Background Glow */}
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-0 pointer-events-none opacity-70">
        <img
          src={logoFull}
          alt=""
          className="w-[800px] blur-3xl filter drop-shadow-[0_0_200px_#00D9FF]"
        />
      </div>
      ...
      <div className="relative z-10 flex flex-col flex-grow">
          ...
          <AnimatePresence mode="wait" initial={false}>
            <Routes location={location} key={isAuthPage ? 'auth' : location.pathname}>
              ...
              <Route element={<ProtectedRoute />}>
                ...
                {/* Locked Routes */}
                <Route path="/shop" element={<PageTransition><FeatureGuard title="SHOP" subtitle="Shop is under construction and we'll let you know when done with it."><Shop /></FeatureGuard></PageTransition>} />
              ...
              <Route path="/tournaments/:id" element={<PageTransition><FeatureGuard title="TOURNAMENTS" subtitle="Tournaments is under construction and we'll let you know when done with it."><TournamentDetails /></FeatureGuard></PageTransition>} />
        ...
        {!isLockedPage && (isAuthPage ? <MinimalFooter /> : <Footer />)}
      </div>
    </div>
  );
};
