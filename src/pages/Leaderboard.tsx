...
import { Search, Filter, ChevronDown, X, Youtube, Gamepad2, Globe, Trophy, Users } from 'lucide-react';
...
const Leaderboard = () => {
    ...
    const games = ['All Games', 'BGMI', 'COD', 'Free Fire', 'Apex Legends'];
    ...
    return (
        <div className="pt-28 min-h-screen bg-[#0B0E14] font-sans pb-20 relative">
            ...
            <div className="container mx-auto px-4 md:px-6 relative z-10 max-w-5xl">
                ...
                {/* PODIUM SECTION - Enhanced Modern Glass Design */}
                ...
                <AnimatePresence>
                    {selectedPlayer && (
                        <div className="fixed inset-0 grid place-items-center z-[100] px-4 pointer-events-none">
                            {/* Backdrop */}
                            <motion.div
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
                                ...
                                className="fixed inset-0 bg-black/80 h-full w-full z-10 backdrop-blur-sm pointer-events-auto"
                            ...
                            {/* Card Container - Responsive Switching */}
                            <motion.div
                                ...
                                className="w-full max-w-[360px] md:max-w-6xl h-auto md:h-[600px] flex flex-col md:flex-row bg-[#0F1116] md:bg-black/40 md:backdrop-blur-xl rounded-[32px] sm:rounded-3xl overflow-hidden z-20 border border-white/10 md:border-white/5 shadow-2xl relative pointer-events-auto"
                            >
                                {/* ================= MOBILE LAYOUT (Vertical Card) ================= */}
                                <div className="flex flex-col md:hidden w-full h-full bg-[#0F1116]">
                                    {/* Image Section */}
                                    <div className="relative h-[380px] w-full">
                                        <motion.div layoutId={`image-${selectedPlayer.name}-${id}`} className="w-full h-full relative">
                                        ...
                                        <div className="absolute bottom-0 left-0 right-0 p-6 flex flex-col gap-1">
                                            <div className="flex items-center gap-2">
                                                <motion.h3 layoutId={`title-${selectedPlayer.name}-${id}`} className="font-bold text-white text-3xl font-display tracking-tight">{selectedPlayer.name}</motion.h3>
                                                <div className="w-5 h-5 rounded-full bg-[#2FE9A9] flex items-center justify-center">
                                                    <svg className="w-3 h-3 text-black fill-current" viewBox="0 0 24 24"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" /></svg>
                                                </div>
                                            </div>
                                            <p className="text-gray-300 text-sm font-medium">Rank <span className="text-white font-bold">#{selectedPlayer.rank}</span> • Score {selectedPlayer.score}</p>
                                        </div>

                                        {/* Floating Socials */}
                                        <div className="absolute top-4 right-4 flex gap-2">
                                            <button className="w-8 h-8 rounded-full bg-black/20 backdrop-blur-md flex items-center justify-center hover:bg-white/20 transition-colors text-white border border-white/10">
                                    ...
                                    <div className="p-6 pt-0 flex flex-col gap-6">
                                        <div className="flex items-center justify-between py-4 border-t border-white/5 border-b mb-2">
                                            <div className="flex items-center gap-2 text-gray-400">
                                                <Users size={16} />
                                                <span className="text-sm font-bold">312 Followers</span>
                                            </div>
                                            <div className="flex items-center gap-2 text-gray-400">
                                                <Trophy size={16} />
                                                <span className="text-sm font-bold">48 Wins</span>
                                            </div>
                                        </div>
                                        <button className="flex-1 h-12 rounded-full font-bold bg-[#E6E6E6] text-black hover:bg-white transition-all shadow-lg flex items-center justify-center gap-2">Follow +</button>
                                    </div>
                                </div>


                                {/* ================= DESKTOP LAYOUT (3-Column Dashboard) ================= */}
                                <div className="hidden md:flex w-full h-full p-8 gap-8 items-start">

                                    {/* COLUMN 1: Profile Identity */}
                                    <div className="w-[28%] flex flex-col items-center text-center h-full border-r border-white/10 pr-8">
                                        {/* Glowing Avatar Ring */}
                                        <div className="relative w-48 h-48 mb-6 group">
                                            <div className="absolute inset-0 rounded-full border-4 border-[#00F0FF] shadow-[0_0_30px_rgba(0,240,255,0.4)] animate-pulse"></div>
                                            <motion.div layoutId={`image-${selectedPlayer.name}-${id}`} className="w-full h-full rounded-full overflow-hidden border-4 border-transparent relative z-10">
                                                <img src={selectedPlayer.img} alt={selectedPlayer.name} className="w-full h-full object-cover" />
                                            </motion.div>
                                        </div>

                                        <motion.h3 layoutId={`title-${selectedPlayer.name}-${id}`} className="font-bold text-white text-3xl font-display tracking-tight mb-1">{selectedPlayer.name}</motion.h3>
                                        <p className="text-[#00F0FF] text-sm font-medium mb-8">@The{selectedPlayer.name.replace(/\s/g, '')}Official</p>

                                        <motion.button
                                            initial={{ opacity: 0, y: 10 }}
                                            animate={{ opacity: 1, y: 0 }}
                                            transition={{ delay: 0.1 }}
                                            className="w-full py-3 rounded-full bg-transparent border border-[#00F0FF] text-[#00F0FF] font-black text-sm tracking-widest hover:bg-[#00F0FF] hover:text-black transition-all shadow-[0_0_20px_rgba(0,240,255,0.2)] mb-8"
                                        >
                                            FOLLOW
                                        </motion.button>

                                        <div className="flex gap-4 justify-center mt-auto">
                                            <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:border-[#00F0FF] hover:shadow-[0_0_15px_rgba(0,240,255,0.3)] transition-all cursor-pointer">
                                                <X size={18} />
                                            </div>
                                            <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-gray-400 hover:text-white hover:border-[#00F0FF] hover:shadow-[0_0_15px_rgba(0,240,255,0.3)] transition-all cursor-pointer">
                                                <Youtube size={18} />
                                    ...
                                    <div className="w-[36%] flex flex-col h-full border-r border-white/10 pr-8">
                                        <h4 className="text-white font-bold text-xl tracking-wider mb-8">ACHIEVEMENTS</h4>

                                        <div className="space-y-8 relative">
                                            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-32 h-64 bg-cyan-500/10 blur-[60px] pointer-events-none"></div>

                                            {/* Stat Bar 1 */}
                                            <div>
                                                <div className="flex justify-between text-xs font-bold text-cyan-400 mb-2 uppercase tracking-wider">
                                                    <span>Rank: GLOBAL ELITE</span>
                                                    <span>Top 1%</span>
                                                </div>
                                                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                                                    <div className="h-full w-[92%] bg-[#00F0FF] shadow-[0_0_10px_#00F0FF]"></div>
                                                </div>
                                            </div>
                                            ...
                                            <div>
                                                <div className="flex justify-between text-xs font-bold text-cyan-400 mb-2 uppercase tracking-wider">
                                                    <span>Total Wins: 1,600+</span>
                                                    <span>Master</span>
                                                </div>
                                                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                                                    <div className="h-full w-[78%] bg-[#00F0FF] shadow-[0_0_10px_#00F0FF]"></div>
                                                </div>
                                            </div>

                                            {/* Stat Bar 3 */}
                                            <div>
                                                <div className="flex justify-between text-xs font-bold text-[#6D44F2] mb-2 uppercase tracking-wider">
                                                    <span>Esports MVP: 12x</span>
                                                    <span>Legendary</span>
                                                </div>
                                                <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                                                    <div className="h-full w-[45%] bg-[#6D44F2] shadow-[0_0_10px_#6D44F2]"></div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex justify-center mt-8">
                                                <div className="relative">
                                                    ...
                                                    <svg className="w-16 h-32 text-cyan-400 drop-shadow-[0_0_8px_rgba(0,240,255,0.8)]" viewBox="0 0 24 24" fill="currentColor">
                                                        <path d="M14.5 13.5L12 16l-2.5-2.5V4h5v9.5zM12 21l-2-2h4l-2 2z" /> {/* Simple sword shape placeholder */}
                                                        <path d="M10 13h4v1h-4zm-2 0h1v4H8zm8 0h-1v4h1z" opacity="0.5" />
                                                    </svg>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex-1 flex flex-col h-full">
                                        <h4 className="text-white font-bold text-xl tracking-wider mb-8">RECENT ACTIVITY</h4>

                                        <div className="space-y-6">
                                            {/* Last Game */}
                                            <div>
                                                <h5 className="text-gray-400 text-xs font-bold uppercase mb-2">Last Game</h5>
                                                <div className="bg-black/40 border border-white/10 rounded-xl overflow-hidden group cursor-pointer hover:border-[#00F0FF]/50 transition-all">
                                                    <div className="h-24 bg-gray-800 relative">
                                                        <img src="https://images.unsplash.com/photo-1542751371-adc38448a05e?w=800&q=80" className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-opacity" />
                                                        <div className="absolute top-2 right-2 px-2 py-0.5 bg-red-500/80 text-white text-[10px] font-bold rounded">LIVE</div>
                                                    </div>
                                                    <div className="p-3">
                                                        <div className="text-white font-bold text-sm">Cyber Assault</div>
                                                        <div className="text-gray-500 text-xs mt-1">Kills 32 / Deaths 8 / KDA 4.0</div>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            {/* Last Tournament */}
                                            <div>
                                                <h5 className="text-gray-400 text-xs font-bold uppercase mb-2">Last Tournament</h5>
                                                <div className="bg-black/40 border border-white/10 rounded-xl overflow-hidden flex items-center p-3 gap-3 group cursor-pointer hover:border-[#00F0FF]/50 transition-all">
                                                    <div className="w-16 h-16 bg-gray-800 rounded-lg overflow-hidden shrink-0">
                                                        <img src="https://images.unsplash.com/photo-1511512578047-dfb367046420?w=800&q=80" className="w-full h-full object-cover" />
                                                    </div>
                                                    <div>
                                                        <div className="text-white font-bold text-sm">Nexus Cup</div>
                                                        <div className="text-gray-500 text-xs">Placement: <span className="text-[#00F0FF]">2nd</span></div>
                                                        <div className="text-gray-500 text-xs">Earnings: <span className="text-[#2FE9A9]">$15,000</span></div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
